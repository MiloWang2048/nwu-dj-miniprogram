...
public class EmployeeDAOTest {
    ...
    public void curdTest() throws InterruptedException {
        Employee rawData = new Employee(null, null, null, null, 1, "test", "test", "test", "test");
        // insert
        int insertResult = employeeDAO.insertOne(rawData);
        assertEquals("数据插入失败", 1, insertResult);
        // find
        Employee findResult = employeeDAO.findById(1);
        ...
        // list
        List<Employee> listResult = employeeDAO.getAllEmployees();
        assertArrayEquals("列表数据与原数据不一致", new Employee[]{rawData}, listResult.toArray());
        // update
        Thread.sleep(1000);
        rawData.setRole("modified");
        int updateResult = employeeDAO.updateWithEntity(rawData);
        assertEquals("update影响行数必须为1", 1, updateResult);
        findResult = employeeDAO.findById(1);
        ...
        // delete
        int deleteResult = employeeDAO.deleteById(1);
